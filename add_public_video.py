...
class AddPublicVideoSpider(BiliobSpider):
        ...
        while True:
            try:
                cursor = self.async_db.author.find({'$or': [{'cFans': {'$gt': 100000}}, {'forceFocus': True}]},
                                                   {'mid': 1}).sort([('biliob.video_update', 1)]).limit(
    ...
    async def parse(self, res):
        try:
            ...
            j = res.json_data
            result = []
            if j == None or 'data' not in j or j['data'] == None or 'list' not in j['data'] or 'vlist' not in j['data'][
                'list']:
                return None
            bvid = None
            aid = None
            ...
            await self.async_db.author.update_one(
                {'mid': mid},
                {'$set': {'channels': channels, 'main_channel': main_channel, 'channel_list': channel_list}})
        ...
        for (bvid, aid, mid) in items:
            ...
            if interval_data == None:
                ...
                await self.async_db.video_interval.update_one(
                    {'bvid': bvid, 'aid': aid}, {'$set': self.update_video_interval(3600 * 24, aid, bvid),
                                                 '$setOnInsert': {'date': datetime.utcnow()}}, upsert=True)
                count += 1
        if mid is not None:
            await self.async_db.author.update_one({'mid': mid}, {'$set': {'biliob.video_update': datetime.utcnow()}},
                                                  upsert=True)
        return count
